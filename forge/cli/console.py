...
calls, and cost as a run unfolds.
"""
...
def make_event_renderer(console: Console):  # type: ignore[no-untyped-def]
    ...
    def render(event: Event) -> None:
        ...
        if event.type is EventType.RUN_STARTED:
            ...
            console.print(
                f"   [magenta]→ {data.get('model')}[/] [dim]{data.get('reason')}[/]"
            )
            ...
            console.print(f"   [green]✓ tool {data.get('tool')}[/]")
        elif event.type is EventType.TOOL_CALL_FAILED:
            console.print(f"   [red]✗ tool {data.get('tool')}: {data.get('error')}[/]")
